...
# -----------------------------------------------------------------------------
# W&B Logging Utilities for HPML Multimodal SLM Training
# All helpers write only from rank 0 to avoid duplicate logs.
...
class WandbLogger:
    """
    Clean W&B logger for HPML training runs.

    Features:
    - Only logs from rank 0 (DDP-aware)
    - Logs system metrics (GPU memory, step time)
    - Logs training and validation metrics
    - Logs full config on init
    - Configurable log interval

    Usage:
        logger = WandbLogger(config=user_config, project="hpml-nanochat-multimodal", ...)
        logger.log(step=100, train_loss=2.5, lr=0.02)
        logger.finish()
    """

    PROJECT_NAME = "hpml-nanochat-multimodal"
    ...
    def __init__(
        ...
        """
        Args:
            config: Full run configuration dict (CLI args, model config, etc.)
            project: W&B project name (default: hpml-nanochat-multimodal)
            name: W&B run name (optional, W&B auto-generates one)
            log_interval: Log every N steps (default: 1)
            enabled: Whether to actually log to W&B (else uses DummyWandb)
            rank: DDP rank (only rank 0 logs)
            world_size: DDP world size
            tags: Optional list of W&B tags
            notes: Optional W&B run notes
        """
        ...
        print(f"W&B initialized: {self._run.url}")

    @staticmethod
    def _clean_config(config: Dict[str, Any]) -> Dict[str, Any]:
        """Remove non-serializable items from config for W&B."""
        import json
    ...
    def set_phase(self, phase: str):
        """Set training phase prefix for metric keys (e.g., 'train', 'val', 'test')."""
        self._phase = phase

    def _prefix_key(self, key: str) -> str:
        """Prefix metric key with phase."""
        if self._phase == "train":
            return key
        return f"{self._phase}/{key}"
    ...
    def log(
        ...
        Args:
            ...
            val_loss: Validation loss (only set during eval)
            lr: Current learning rate
            tokens_per_second: Throughput metric
            samples_per_second: Throughput metric (batch-level)
            step_time_ms: Step time in milliseconds
            gpu_memory_allocated_mb: Current GPU memory allocated
            gpu_memory_reserved_mb: Peak GPU memory reserved
            grad_norm: Gradient norm (before clipping)
            mfu_pct: Model FLOPS utilization percentage
        ...
        if tokens_per_second is not None:
            metrics["train/tokens_per_second"] = tokens_per_second
        if samples_per_second is not None:
            metrics["train/samples_per_second"] = samples_per_second
        if step_time_ms is not None:
            metrics["train/step_time_ms"] = step_time_ms
        if gpu_memory_allocated_mb is not None:
            metrics["system/gpu_memory_allocated_mb"] = gpu_memory_allocated_mb
        if gpu_memory_reserved_mb is not None:
            metrics["system/gpu_memory_reserved_mb"] = gpu_memory_reserved_mb
        if grad_norm is not None:
            metrics["train/grad_norm"] = grad_norm
        if mfu_pct is not None:
            metrics["train/mfu_pct"] = mfu_pct
    ...
    def log_summary(self, metrics: Dict[str, Any]):
        """Log final summary metrics (run after training)."""
        if not self.is_master:
            return
    ...
    def watch(self, model):
        """Call wandb.watch on the model (only on master)."""
        if self.is_master:
            self._run.watch(model)


def get_gpu_memory_mb(device: str = "cuda") -> tuple:
    """
    Get current GPU memory allocated and reserved (MB).

    Returns:
        (allocated_mb, reserved_mb) tuple
    """
    if not torch.cuda.is_available():
        return 0.0, 0.0
...
def get_grad_norm(model, world_size: int = 1) -> float:
...
class DummyWandb:
    """Dummy W&B logger for non-master ranks or when disabled."""
...
def build_wandb_logger(
    ...
    """
    Convenience factory to build a WandbLogger.

    Checks DDP rank and only initializes on rank 0.

    Args:
        config: Full run configuration (CLI args + model config dict)
        rank: DDP rank
        world_size: DDP world size
        log_interval: Log every N steps
        wandb_run_name: Optional W&B run name
        wandb_tags: Optional W&B tags
        wandb_notes: Optional W&B run notes
        wandb_enabled: Whether to enable W&B (False = DummyWandb)
        project: W&B project name

    Returns:
        WandbLogger (or DummyWandb on non-master ranks)
    """
    return WandbLogger(
        config=config,
        project=project,
